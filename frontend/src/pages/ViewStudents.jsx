...
import { Plus, Search, Eye, Loader2, Trash2, RefreshCw, X } from 'lucide-react';
import api from '../api';
...
const ViewStudents = () => {
  ...
  const load = () => {
    ...
    api.get('/students').then(({ data }) => { setStudents(data); setLoading(false); }).catch(() => { setError('Students could not be loaded. Please make sure the backend is running, then retry.'); setLoading(false); });
  };
  useEffect(load, []);
    ...
    <Layout>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex flex-wrap items-center gap-3 mb-4">
          ...
          <select value={selectedClass} onChange={(e) => { setSelectedClass(e.target.value); setSelectedSection(''); }} className="h-10 rounded-lg border border-gray-200 bg-white px-3 text-[13px] text-[#555] focus:outline-none focus:ring-2 focus:ring-red-100">
            <option value="">All Classes</option>{classes.map((value) => <option key={value} value={value}>{value}</option>)}
          </select>
          <select value={selectedSection} onChange={(e) => setSelectedSection(e.target.value)} className="h-10 rounded-lg border border-gray-200 bg-white px-3 text-[13px] text-[#555] focus:outline-none focus:ring-2 focus:ring-red-100">
            <option value="">All Sections</option>{sections.map((value) => <option key={value} value={value}>{value}</option>)}
          </select>
          {(q || selectedClass || selectedSection) && <button onClick={clearFilters} className="inline-flex items-center gap-1.5 h-10 px-3 rounded-lg text-[13px] text-[#C4141B] hover:bg-red-50"><X className="w-4 h-4" /> Clear</button>}
        </div>

        {error && <div className="mb-4 flex items-center justify-between rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-[13px] text-[#a91116]"><span>{error}</span><button onClick={load} className="inline-flex items-center gap-1.5 font-semibold"><RefreshCw className="w-4 h-4" /> Retry</button></div>}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="text-[11px] uppercase tracking-wide text-[#9a9a9a] border-b border-gray-100">
                <th className="py-3 font-medium">Student</th><th className="py-3 font-medium">ID</th><th className="py-3 font-medium">Class</th><th className="py-3 font-medium">Section</th><th className="py-3 font-medium">Roll</th><th className="py-3 font-medium">Status</th><th className="py-3 font-medium text-right">Action</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((s) => (
                <tr key={s.id} className="border-b border-gray-50 hover:bg-[#fafafa] transition">
                  ...
                  <td className="py-3 text-[13px] text-[#666]">{s.class_name || '—'}</td>
                  <td className="py-3 text-[13px] text-[#666]">{s.section || '—'}</td>
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="py-10 text-center text-[13px] text-[#999]">{students.length ? 'No students match your search or filters.' : 'No students found.'}</td></tr>}
            </tbody>
          </table>
        )}
      </div>
